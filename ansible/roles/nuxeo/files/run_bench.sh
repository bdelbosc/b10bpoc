...
function load_data_into_redis() {
  ...
  wget -qO- http://maven.nuxeo.org/nexus/content/groups/public/org/nuxeo/tools/testing/data-test-les-arbres/1.1/data-test-les-arbres-1.1.zip | funzip > ~/data/les-arbres.csv
...
function gatling() {
  mvn -nsu test gatling:execute -Pqa -Pbench -Durl=$TARGET -Dgatling.simulationClass=$@
}
...
function run_simulations() {
  ...
  #curl -XPOST http://monitor1:8080/events/  -d '{"what": "Gatling search 1 user", "tags":"phases gatling"}'
  #gatling "org.nuxeo.cap.bench.Sim30Search" -Dusers=1 -Dduration=180
  curl -XPOST http://monitor1:8080/events/  -d '{"what": "Gatling search 100 users", "tags":"phases gatling"}'
  gatling "org.nuxeo.cap.bench.Sim30Search" -Dusers=100 -Dduration=300
  curl -XPOST http://monitor1:8080/events/  -d '{"what": "Gatling terminated", "tags":"phases gatling"}'
  popd
}
...
function run_all_simulations() {
  ...
  mvn -nsu clean -Pqa
  # init the users/group on all instance so we don't need to share a directory db
  curl -XPOST http://monitor1:8080/events/  -d '{"what": "Gatling setup", "tags":"phases gatling"}'
  mvn -nsu -Pqa test gatling:execute -Pbench -Durl=http://172.31.14.232:8080/nuxeo -Dgatling.simulationClass=org.nuxeo.cap.bench.Sim00Setup
  mvn -nsu -Pqa test gatling:execute -Pbench -Durl=http://172.31.10.37:8080/nuxeo -Dgatling.simulationClass=org.nuxeo.cap.bench.Sim00Setup
  ...
  #curl -XPOST http://monitor1:8080/events/  -d '{"what": "Gatling import", "tags":"phases gatling"}'
  ...
  curl -XPOST http://monitor1:8080/events/  -d '{"what": "Gatling create folders", "tags":"phases gatling"}'
  gatling "org.nuxeo.cap.bench.Sim10CreateFolders"
  curl -XPOST http://monitor1:8080/events/  -d '{"what": "Gatling create documents", "tags":"phases gatling"}'
  gatling "org.nuxeo.cap.bench.Sim20CreateDocuments" -Dusers=$CONCURRENT_USERS
  curl -XPOST http://monitor1:8080/events/  -d '{"what": "Gatling create documents async", "tags":"phases gatling"}'
  gatling "org.nuxeo.cap.bench.Sim25WaitForAsync"
  curl -XPOST http://monitor1:8080/events/  -d '{"what": "Gatling update documents", "tags":"phases gatling"}'
  ...
  curl -XPOST http://monitor1:8080/events/  -d '{"what": "Gatling update documents async", "tags":"phases gatling"}'
  gatling "org.nuxeo.cap.bench.Sim35WaitForAsync"
  curl -XPOST http://monitor1:8080/events/  -d '{"what": "Gatling navigation", "tags":"phases gatling"}'
  gatling "org.nuxeo.cap.bench.Sim30Navigation" -Dusers=$CONCURRENT_USERS -Dduration=180
  curl -XPOST http://monitor1:8080/events/  -d '{"what": "Gatling search", "tags":"phases gatling"}'
  gatling "org.nuxeo.cap.bench.Sim30Search" -Dusers=$CONCURRENT_USERS -Dduration=180
  #curl -XPOST http://monitor1:8080/events/  -d '{"what": "Gatling navigation jsf", "tags":"phases gatling"}'
  #gatling "org.nuxeo.cap.bench.Sim30NavigationJsf" -Dduration=180
  curl -XPOST http://monitor1:8080/events/  -d '{"what": "Gatling bench", "tags":"phases gatling"}'
  gatling "org.nuxeo.cap.bench.Sim50Bench" -Dnav.users=80 -Dnavjsf=5 -Dupd.user=15 -Dnavjsf.pause_ms=1000 -Dduration=180
  curl -XPOST http://monitor1:8080/events/  -d '{"what": "Gatling CRUD", "tags":"phases gatling"}'
  gatling "org.nuxeo.cap.bench.Sim50CRUD" -Dusers=$CONCURRENT_USERS -Dduration=120
  curl -XPOST http://monitor1:8080/events/  -d '{"what": "Gatling CRUD async", "tags":"phases gatling"}'
...
function build_stat() {
  ...
  java -jar $GAT_REPORT_JAR -f -o $REPORT_PATH -n data.yml -t $MUSTACHE_TEMPLATE \
    -m create,createasync,nav,search,update,updateasync,bench,crud,crudasync \
...
#    $REPORT_PATH/sim30navigationjsf/detail/simulation.log.gz \
  ...
  echo "nuxeonodes: 2" >> $REPORT_PATH/data.yml
  echo "dbnodes: 3" >> $REPORT_PATH/data.yml
  echo "esnodes: 1" >> $REPORT_PATH/data.yml
  echo "nuxeotype: \"c5.4xlarge\"" >> $REPORT_PATH/data.yml
  echo "dbtype: \"m5.4xlarge\"" >> $REPORT_PATH/data.yml
  echo "estype: \"c5.9xlarge\"" >> $REPORT_PATH/data.yml
  echo "distribution: \"http://community.nuxeo.com/static/snapshots/nuxeo-server-tomcat-10.2-SNAPSHOT.zip\"" >> $REPORT_PATH/data.yml
  echo "bench_suite: \"a500m\"" >> $REPORT_PATH/data.yml
  echo "classifier: \"180417-1216_2nuxeo-c5.4xlarge_3mongo-m5.4xlarge_1es-c5.9xlarge\"" >> $REPORT_PATH/data.yml
